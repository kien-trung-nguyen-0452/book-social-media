...
public class CommentServiceImpl implements CommentService {
    ...
    public void deleteByUserIdWithoutAuth(String userId) {
        List<Comment> comments = repository.findByUsername(userId);
        for (Comment comment : comments) {
            deleteRepliesRecursive(comment.getId());
            repository.deleteById(comment.getId());
        }
        log.info("Deleted all comments for user {}", userId);
    }
